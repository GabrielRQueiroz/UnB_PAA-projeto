...
export const Results = () => {
  ...
  const generateImage = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!inputRef.current?.value) {
      return toast.warning('Digite algo para gerar a imagem.');
    }
  ...
  return (
    <div className="mt-80 mx-auto flex flex-col gap-4 w-4/5 text-lg ">
      ...
      <section className="flex flex-col gap-4 min-h-48 items-center justify-center pt-6 pb-16 overflow-x-hidden">
        {
          isLoading ?
            <div className="flex flex-col w-96 h-96 items-center justify-center gap-16">
              <div className="animate-spin relative w-16 h-16 border-4 border-t-blue-800 border-r-blue-600 border-b-blue-400 border-l-blue-200 border-solid rounded-full" />
              <button
                onClick={() => {
                  controller.abort()
                }}
                className="bg-stone-950 relative mx-auto py-2 px-10 border-2 border-blue-900 rounded-md transition-all duration text-indigo-200 hover:bg-blue-900"
              >
                Cancelar
                ✖️
              </button>
